...
TopDownHierModel::TopDownHierModel(TopDownPar *par) : models(2)
{
    ...
    switch (((ModelPar *)(par->GetModelPar(0)))->GetModelType()) {
        ...
        case AS_BARABASI_1:
            as_bar_1_model = new ASBarabasiAlbert_1(
                (ASBarabasiAlbert_1_Par *)(par->GetModelPar(0)));
            models[0] = as_bar_1_model;
            break;

        case AS_BARABASI_2:
            as_bar_2_model = new ASBarabasiAlbert_2(
                (ASBarabasiAlbert_2_Par *)(par->GetModelPar(0)));
            models[0] = as_bar_2_model;
            break;

        case IF_AS:

            switch ((int)((ImportedFilePar *)(par->GetModelPar(0)))->GetFormat()) {
                case ImportedFileModel::IF_BRITE:
                    cout << "Importing brite...\n" << flush;
                    if_brite_model = new ImportedBriteTopologyModel(
                        (ImportedFilePar *)(par->GetModelPar(0)));
                ...
                case ImportedFileModel::IF_GTITM_TS:
                    ...
                    if_gtitm_model = new ImportedGTitmTopologyModel(
                        (ImportedFilePar *)(par->GetModelPar(0)));
                ...
                case ImportedFileModel::IF_NLANR:
                    ...
                    if_nlanr_model = new ImportedNLANRTopologyModel(
                        (ImportedFilePar *)(par->GetModelPar(0)));
    ...
    switch (par->GetModelPar(1)->GetModelType()) {
        ...
        case RT_BARABASI_1:
            rt_bar_1_model = new RouterBarabasiAlbert_1(
                (RouterBarabasiAlbert_1_Par *)(par->GetModelPar(1)));
            models[1] = rt_bar_1_model;
            break;

        case RT_BARABASI_2:
            rt_bar_2_model = new RouterBarabasiAlbert_2(
                (RouterBarabasiAlbert_2_Par *)(par->GetModelPar(1)));
        ...
        case IF_ROUTER:
            ...
            switch (((ImportedFilePar *)(par->GetModelPar(1)))->GetFormat()) {
                case ImportedFileModel::IF_BRITE:
                    cout << "Importing brite...\n" << flush;
                    if_brite_model = new ImportedBriteTopologyModel(
                        (ImportedFilePar *)(par->GetModelPar(1)));
...
string TopDownHierModel::ToString()
{
    ...
    os << "Model ( 5 ): " << (int)edge_conn_type << " " << k << " " << Scale_1 << " " << Scale_2
       << "\n";
...
void TopDownHierModel::InterConnectBorders(Graph *g, Graph *flat_g)
{
    ...
    for (int i = 1; i < g->GetNumNodes(); i++) {
        num_nodes_i =
            ((ASNodeConf *)g->GetNodePtr(i)->GetNodeInfo())->GetTopology()->GetNumNodes();
...
int TopDownHierModel::GetFlatRandomNode(int ASid, Graph *g, Graph *flat_g, vector<int> &p,
                                        RandomVariable &U)
{
    int n;
...
int TopDownHierModel::GetFlatSmallest(int ASid, Graph *g, Graph *flat_g, vector<int> &p)
{
    int up;
...
int TopDownHierModel::GetFlatSmallestNoLeaf(int ASid, Graph *g, Graph *flat_g, vector<int> &p)
{
    int up;
    ...
    for (int i = p[ASid]; i < up; i++) {
        Node *node = flat_g->GetNodePtr(i);
        if ((node->GetOutDegree() < smallest) && (node->GetOutDegree() > min)) {
            n = i;
            smallest = node->GetOutDegree();
        }
    }
...
int TopDownHierModel::GetFlatSmallestK(int ASid, Graph *g, Graph *flat_g, vector<int> &p)
{
    int up;
    ...
    for (int i = p[ASid]; i < up; i++) {
        Node *node = flat_g->GetNodePtr(i);
        if ((node->GetOutDegree() < smallest) && (node->GetOutDegree() > k)) {
            n = i;
            smallest = node->GetOutDegree();
        }
    }
