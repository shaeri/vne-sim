...
ImportedInetTopologyModel::ImportedInetTopologyModel(ImportedFilePar *par) : ImportedFileModel(par)
{
}
...
void ImportedFileModel::AssignBW(Graph *g)
{
    ...
    for (el = g->edges.begin(); el != g->edges.end(); el++) {
        switch (BWdist) {
            ...
            default:
                cerr << "ImportedFileModel::AssignBW():  invalid BW distribution (" << (int)BWdist
                     << ")...\n"
                     << flush;
        }
...
string ImportedFileModel::ToString()
{
    ...
    os << " " << (int)format << " " << filename << " " << (int)GetBWDist() << " " << GetBWMin()
       << " " << GetBWMax() << "\n"
       << '\0';
...
Graph *ImportedBriteTopologyModel::ParseFile()
{
    ...
    while ((i = p.GetNextTokenList(toks)) == 0)
        ;
    if (toks[0] != (char *)"Topology:")
        p.ParseError((char *)"Topology:", toks[0]);
    if (toks[1] != (char *)"(")
        p.ParseError((char *)"(", toks[1]);
    int nodes = atoi(toks[2].c_str());
    assert(nodes > 0);
    if (toks[3] != (char *)"Nodes,")
        p.ParseError((char *)"Nodes,", toks[3]);
    int edges = atoi(toks[4].c_str());
    assert(edges > 0);
    if (toks[5] != (char *)"Edges")
        p.ParseError((char *)"Edges", toks[5]);
    if (toks[6] != (char *)")")
        p.ParseError((char *)")", toks[6]);
    ...
    while (toks[0] != "Nodes:") {
        while ((i = p.GetNextTokenList(toks)) == 0)
            ;
        if (i < 0)
            p.ParseError((char *)"Model information", "EOF");
        if (toks[0] != "Nodes:") {
            for (int j = 0; j < i; j++) {
                model_string += toks[i] + " ";
            }
    ...
    for (int j = 0; j < nodes; j++) {
        while ((i = p.GetNextTokenList(toks)) == 0)
            ;
        if (i < 0)
            p.ParseError((char *)"Node lines", "EOF");
	      ...
	      node->SetOutDegree(odeg);*/
            ...
            switch (level) {
                case ImportedFileModel::RT_LEVEL:
                    ...
                    if (type == "RT_BACKBONE")
                        n_rt_conf->SetRouterType(RouterNodeConf::RT_BACKBONE);
                    else if (type == "RT_NONE")
                        n_rt_conf->SetRouterType(RouterNodeConf::RT_NONE);
                    else if (type == "RT_BORDER")
                        n_rt_conf->SetRouterType(RouterNodeConf::RT_BORDER);
                    else if (type == "RT_STUB")
                        n_rt_conf->SetRouterType(RouterNodeConf::RT_STUB);
                    else if (type == "RT_LEAF")
                        n_rt_conf->SetRouterType(RouterNodeConf::RT_LEAF);
                    else {
                        cerr << "ImportBriteTopology(): Invalid Router node type (" << type << ") "
                             << nid << "  read...\n"
                             << flush;
                        exit(0);
                    }
                ...
                case ImportedFileModel::AS_LEVEL:
                    ...
                    if (type == "AS_BACKBONE")
                        n_as_conf->SetASType(ASNodeConf::AS_BACKBONE);
                    else if (type == "AS_BORDER")
                        n_as_conf->SetASType(ASNodeConf::AS_BORDER);
                    else if (type == "AS_NONE")
                        n_as_conf->SetASType(ASNodeConf::AS_NONE);
                    else if (type == "AS_STUB")
                        n_as_conf->SetASType(ASNodeConf::AS_STUB);
                    else if (type == "AS_LEAF")
                        n_as_conf->SetASType(ASNodeConf::AS_LEAF);
                    else {
                        cerr << "ImportBriteTopology(): Invalid AS node type read...\n" << flush;
                        exit(0);
                    }
                    n_as_conf->SetASId(j);
    ...
    while ((i = p.GetNextTokenList(toks)) == 0)
        ;
    if (i < 0)
        p.ParseError((char *)"Edges", "EOF");
    if (toks[0] != (char *)"Edges:")
        p.ParseError((char *)"Edges:", toks[0]);

    for (int j = 0; j < edges; j++) {
        while ((i = p.GetNextTokenList(toks)) == 0)
            ;
        if (i < 0)
            p.ParseError((char *)"Edge information", "EOF");
        ...
        try {
            ...
            switch (level) {
                case ImportedFileModel::RT_LEVEL:
                    ...
                    if (type == "E_RT_BACKBONE")
                        e_rt_conf->SetRouterEdgeType(RouterEdgeConf::RT_BACKBONE);
                    else if (type == "E_RT_BORDER")
                        e_rt_conf->SetRouterEdgeType(RouterEdgeConf::RT_BORDER);
                    else if (type == "E_RT_STUB")
                        e_rt_conf->SetRouterEdgeType(RouterEdgeConf::RT_STUB);
                    else if (type == "E_RT_NONE")
                        e_rt_conf->SetRouterEdgeType(RouterEdgeConf::RT_NONE);
                    else {
                        cerr << "ImportBriteTopology(): Invalid Router edge type read...\n"
                             << flush;
                        exit(0);
                    }
                ...
                case ImportedFileModel::AS_LEVEL:
                    ...
                    if (type == "E_AS_BACKBONE")
                        e_as_conf->SetASEdgeType(ASEdgeConf::AS_BACKBONE);
                    else if (type == "E_AS_BORDER")
                        e_as_conf->SetASEdgeType(ASEdgeConf::AS_BORDER);
                    else if (type == "E_AS_STUB")
                        e_as_conf->SetASEdgeType(ASEdgeConf::AS_STUB);
                    else if (type == "E_AS_NONE")
                        e_as_conf->SetASEdgeType(ASEdgeConf::AS_NONE);
                    else {
                        cerr << "ImportBriteTopology(): Invalid AS edge type read...\n" << flush;
                        exit(0);
                    }
...
Graph *ImportedGTitmTopologyModel::ParseFlatGTITM()
{
    ...
    while ((i = p.GetNextTokenList(toks)) == 0)
        ;
    if (toks[0] != (char *)"GRAPH")
        p.ParseError((char *)"GRAPH", toks[0]);
    if (toks[1] != (char *)"(#nodes")
        p.ParseError((char *)"(#nodes", toks[1]);
    ...
    while ((i = p.GetNextTokenList(toks)) == 0)
        ;
    if (toks[0] != (char *)"VERTICES")
        p.ParseError((char *)"VERTICES", toks[0]);
...
Graph *ImportedGTitmTopologyModel::ParseTSGTITM()
{
    ...
    if (toks[2].compare(string("transtub", 0, 8)) != 0)
        p.ParseError((char *)"n m transtub(...", toks[2]);
...
Graph *ImportedNLANRTopologyModel::ParseFile()
{
    ...
    while ((i = p.GetNextToken(node_string)) != 0) {
        ...
        p.ParseStringField((char *)"->");
        p.ParseIntField(degree);

        for (int j = 0; j < degree; j++) {
            i = p.GetNextToken(neighbor_string);
            if (i == 0)
                p.ParseError((char *)"int node id", neighbor_string);
