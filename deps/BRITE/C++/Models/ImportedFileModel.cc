...
Graph* ImportedBriteTopologyModel::ParseFile() {
  ...
  if (toks[0] != (char*) "Topology:") p.ParseError((char*) "Topology:", toks[0]);
  if (toks[1] != (char*) "(") p.ParseError((char*) "(", toks[1]);
  int nodes = atoi(toks[2].c_str());
  assert(nodes > 0);
  if (toks[3] != (char*) "Nodes,") p.ParseError((char*) "Nodes,", toks[3]);
  int edges = atoi(toks[4].c_str());
  assert(edges > 0);
  if (toks[5] != (char*) "Edges") p.ParseError((char*) "Edges", toks[5]);
  if (toks[6] != (char*) ")") p.ParseError((char*) ")", toks[6]);
  ...
  while (toks[0] != "Nodes:") {
    ...
    while ((i = p.GetNextTokenList(toks)) == 0);
    if (i < 0) p.ParseError((char*) "Model information", "EOF");
    if (toks[0] != "Nodes:") {
      for (int j = 0; j < i; j++) {
	model_string += toks[i] + " ";
      }
  ...
  for (int j = 0; j < nodes; j++) {
    ...
    while ((i = p.GetNextTokenList(toks)) == 0);
    if (i < 0) p.ParseError((char*) "Node lines", "EOF");
  ...
  /* Parse Edges */
  while ((i = p.GetNextTokenList(toks)) == 0);
  if (i < 0) p.ParseError((char*) "Edges", "EOF");
  if (toks[0] != (char*) "Edges:") p.ParseError((char*) "Edges:", toks[0]);
  
  for (int j = 0; j < edges; j++) {
    
    while ((i = p.GetNextTokenList(toks)) == 0);
    if (i < 0) p.ParseError((char*) "Edge information", "EOF");
...
Graph* ImportedGTitmTopologyModel::ParseFlatGTITM() {
  ...
  /* Parse headers */
  while ((i = p.GetNextTokenList(toks)) == 0);
  if (toks[0] != (char*) "GRAPH") p.ParseError((char*) "GRAPH", toks[0]);
  if (toks[1] != (char*) "(#nodes") p.ParseError((char*) "(#nodes", toks[1]);
  ...
  cout << "Parsing nodes...\n" << flush;
  while ((i = p.GetNextTokenList(toks)) == 0);
  if (toks[0] != (char*) "VERTICES") p.ParseError((char*) "VERTICES", toks[0]);
...
Graph* ImportedNLANRTopologyModel::ParseFile() {
  ...
  while ((i = p.GetNextTokenList(toks)) > 0) { 
    ...
    if ((i == 0) || (toks[1] != "->")) p.ParseError((char*) "->", toks[1]);
    auxid[nid] =  toks[0];
  ...
  while ((i = p.GetNextToken(node_string)) != 0) { 
    ...
    for (int j = 0; j < degree; j++) {
      ...
      if (i == 0) p.ParseError((char*) "int node id", neighbor_string);
