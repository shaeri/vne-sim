...
BottomUpHierModel::BottomUpHierModel(BottUpPar *par) : models(2)
{
    ...
    switch (par->GetModelPar(0)->GetModelType()) {
        ...
        case RT_BARABASI_1:
            rt_bar_model = new RouterBarabasiAlbert_1(
                (RouterBarabasiAlbert_1_Par *)(par->GetModelPar(0)));
...
string BottomUpHierModel::ToString()
{
    ...
    os << "Model ( 6 ): " << ASNodes << " " << (int)group << " " << (int)at << " "
       << (int)BWInterdist << " " << BWIntermin << " " << BWIntermax << "\n";
...
void BottomUpHierModel::RandomWalk(Graph *g, vector<Color> &color, int u, int size,
                                   RandomVariable &U, int AS, int &c)
{
    int v, l;
    ...
    for (el = g->edges.begin(); el != g->edges.end(); el++) {
        ...
        if (ASFrom != ASTo) {
            switch (GetBWInterDist()) {
                ...
                default:
                    cerr << "BUHier::AssignBW():  invalid BW distribution (" << GetBWInterDist()
                         << ")...\n"
                         << flush;
                    exit(0);
            }
