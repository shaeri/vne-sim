...
string ASBarabasiAlbert_2::ToString()
{
    ...
    os << "Model ( 10 ): " << size << " " << Scale_1 << " " << Scale_2 << " " << (int)NodePlacement
       << " " << m_edges << " " << (int)GetBWDist() << " " << GetBWMin() << " " << GetBWMax()
       << " " << P << " " << Q << '\0';
    ...
    while (added_nodes < g->GetNumNodes() - 1) {
        ...
        if (r < P) { /* Add m new edges */
            ...
            while (num_edges_added < m_edges) {
                ...
                if (srcind == dstind)
                    continue;
                if (g->AdjListFind(srcind, dstind) || g->AdjListFind(srcind, dstind))
                    continue;
        ...
        if ((r > P) && (r < P + Q)) { /* rewire m edges */
            ...
            while (num_edges_rewired < m_edges) {
                ...
                if (dstind == random_src_index)
                    continue;
                if (g->AdjListFind(random_src_index, dstind))
                    continue;
