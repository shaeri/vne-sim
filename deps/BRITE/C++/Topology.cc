...
/*              Anukool Lakhina                                             */
/*  Modified:   Soroush Haeri 5/04/2015: resolved some compiler warnings    */
...
void Topology::BriteOutput(char *filename)
{
    ...
    outfile << "Topology: ( " << g->GetNumNodes() << " Nodes, " << g->GetNumEdges()
            << " Edges )\n";
    ...
    switch (m->GetType()) {
        ...
        default:
            cerr << "Topology::Output(): Invalid model type (" << (int)m->GetType()
                 << ")  passed....\n";
            exit(0);
    }
    ...
    for (int i = 0; i < g->GetNumNodes(); i++) {
        outfile << g->GetNodePtr(i)->GetId() << " " << g->GetNodePtr(i)->GetNodeInfo()->GetCoordX()
                << " " << g->GetNodePtr(i)->GetNodeInfo()->GetCoordY() << " "
                << g->GetNodePtr(i)->GetInDegree() << " " << g->GetNodePtr(i)->GetOutDegree()
                << " ";
            ...
            case NodeConf::RT_NODE:
                ...
                switch (((RouterNodeConf *)(g->GetNodePtr(i)->GetNodeInfo()))->GetRouterType()) {
                    ...
                    case RouterNodeConf::RT_BACKBONE:
                        ...
                        break;
                    default:
                        cerr << "Topology::Output(): Improperly classfied Router node "
                                "encountered...\n";
                        assert(0);
                }
                break;
    ...
    for (el = g->edges.begin(); el != g->edges.end(); el++) {
        outfile << (*el)->GetId() << " " << (*el)->GetSrc()->GetId() << " "
                << (*el)->GetDst()->GetId() << " " << (*el)->Length() << " ";
        ...
        switch ((*el)->GetConf()->GetEdgeType()) {
            ...
            case EdgeConf::AS_EDGE:
                outfile << -1 /* No delay for AS Edges */ << " " << (*el)->GetConf()->GetBW()
                        << " "
                        //	       << (*el)->GetConf()->GetWeight() << " "
...
void Topology::OtterOutput(char *filename)
{
    ...
    list<Edge *>::iterator el;
    for (el = g->edges.begin(); el != g->edges.end(); el++) {
        outfile << "l " << (*el)->GetId() << " " << (*el)->GetSrc()->GetId() << " "
                << (*el)->GetDst()->GetId() << " \" \"\n";
    }
...
void Topology::Classify()
{
    ...
    for (int i = 0; i < g->GetNumNodes(); i++) {
        ...
        if (node->GetOutDegree() <= m->GetMEdges()) {
            switch (node->GetNodeInfo()->GetNodeType()) {
                case NodeConf::RT_NODE:
                    ((RouterNodeConf *)(node->GetNodeInfo()))
                        ->SetRouterType(RouterNodeConf::RT_LEAF);
                    break;
    ...
    for (el = g->edges.begin(); el != g->edges.end(); el++) {
        ...
        if ((Src->GetNodeInfo()->GetNodeType() == NodeConf::RT_NODE &&
            ...
            switch ((*el)->GetConf()->GetEdgeType()) {
                case EdgeConf::RT_EDGE:
                    ((RouterEdgeConf *)((*el)->GetConf()))
                        ->SetRouterEdgeType(RouterEdgeConf::RT_STUB);
                    break;
                case EdgeConf::AS_EDGE:
    ...
    for (int i = 0; i < g->GetNumNodes(); i++) {
        ...
        if (((node->GetNodeInfo()->GetNodeType() == NodeConf::RT_NODE) &&
             (((RouterNodeConf *)(node->GetNodeInfo()))->GetRouterType() ==
              RouterNodeConf::RT_LEAF)) ||
        ...
        for (el = g->incList[i].begin(); el != g->incList[i].end(); el++) {
            if (((((EdgeConf *)((*el)->GetConf()))->GetEdgeType() == EdgeConf::AS_EDGE) &&
                ...
                ((((EdgeConf *)((*el)->GetConf()))->GetEdgeType() == EdgeConf::RT_EDGE) &&
                 (((RouterEdgeConf *)((*el)->GetConf()))->GetRouterEdgeType() ==
                  RouterEdgeConf::RT_STUB))) {
                num_stub_links += 1;
            }
        }
        ...
        switch (node->GetNodeInfo()->GetNodeType()) {
            case NodeConf::RT_NODE:
                if (num_stub_links == 1) {
                    ((RouterNodeConf *)(node->GetNodeInfo()))
                        ->SetRouterType(RouterNodeConf::RT_STUB);
                }
                if (num_stub_links > 1) {
                    ((RouterNodeConf *)(node->GetNodeInfo()))
                        ->SetRouterType(RouterNodeConf::RT_BORDER);
                }
                if (num_stub_links == 0) {
                    ((RouterNodeConf *)(node->GetNodeInfo()))
                        ->SetRouterType(RouterNodeConf::RT_BACKBONE);
                }
                break;
        ...
        if ((((*el)->GetConf()->GetEdgeType() == EdgeConf::AS_EDGE) &&
            ...
            (((*el)->GetConf()->GetEdgeType() == EdgeConf::RT_EDGE) &&
             (((RouterEdgeConf *)((*el)->GetConf()))->GetRouterEdgeType() !=
              RouterEdgeConf::RT_STUB))) {
            if (Src->GetNodeInfo()->GetNodeType() == NodeConf::RT_NODE) {
                if (((((RouterNodeConf *)(Src->GetNodeInfo()))->GetRouterType() ==
                      RouterNodeConf::RT_STUB) &&
                     ((RouterNodeConf *)(Dst->GetNodeInfo()))->GetRouterType() ==
                         RouterNodeConf::RT_BORDER) ||
                    ((((RouterNodeConf *)(Src->GetNodeInfo()))->GetRouterType() ==
                      RouterNodeConf::RT_BORDER) &&
                     ((RouterNodeConf *)(Dst->GetNodeInfo()))->GetRouterType() ==
                         RouterNodeConf::RT_STUB)) {
                    ((RouterEdgeConf *)((*el)->GetConf()))
                        ->SetRouterEdgeType(RouterEdgeConf::RT_BORDER);

                } else {
                    ((RouterEdgeConf *)((*el)->GetConf()))
                        ->SetRouterEdgeType(RouterEdgeConf::RT_BACKBONE);
                }

            } else {
                if (((((ASNodeConf *)(Src->GetNodeInfo()))->GetASType() == ASNodeConf::AS_STUB) &&
                     ((ASNodeConf *)(Dst->GetNodeInfo()))->GetASType() == ASNodeConf::AS_BORDER) ||
                    ((((ASNodeConf *)(Src->GetNodeInfo()))->GetASType() ==
                      ASNodeConf::AS_BORDER) &&
