...
namespace vineyard
{
    ...
    VYSubstrateNetworkBuilder<>::getNetwork()
    {
        ...
        snfilePath << ConfigManager::Instance()->getConfig<std::string>("vineyard",
                                                                        "SubstrateNetwork", "path")
                   << ("/")
        ...
        try {
            ...
            for (int i = 0; i < numEdges; i++) {
                ...
                BOOST_LOG_TRIVIAL(debug)
                    << from << " " << to << " " << bw << " " << delay << std::endl;
                std::shared_ptr<VYSubstrateLink<>> l(
                    new VYSubstrateLink<>(bw, delay, from + nodeId_offset, to + nodeId_offset));
                _net->addLink(l);
            }
            snFile.close();
            return _net;
