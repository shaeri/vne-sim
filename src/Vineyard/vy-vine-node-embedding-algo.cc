...
namespace vineyard
{
          ...
          virtualNodeIdSet(nullptr)
    {
        ...
        LPdataFile = ConfigManager::Instance()->getConfig<std::string>("vineyard", "glpk",
                                                                       "LPdataFile");
        LPmodelFile = ConfigManager::Instance()->getConfig<std::string>("vineyard", "glpk",
                                                                        "LPmodelFile");
        ...
        if (ConfigManager::Instance()
                ->getConfig<std::string>("vineyard", "Configs", "nodeMappingType")
                .compare("randomized") == 0)
        ...
        std::shared_ptr<SUBSTRATE_TYPE> substrate_network, std::shared_ptr<VNR_TYPE> vnr)
    {
        ...
        std::vector<std::vector<double>> bVec(
            (substrateNodesNum + virtualNodeNum),
            std::vector<double>(substrateNodesNum + virtualNodeNum));
        ...
        for (int i = 0; i < virtualNodeNum; i++) {
            fprintf(fpDat, "%d %.4lf\n", substrateNodesNum + i,
                    vnr->getVN()->getNode(allNodeIds[substrateNodesNum + i])->getCPU());
        }
        fprintf(fpDat, ";\n\n");
        ...
        for (int i = 0; i < (substrateNodesNum + virtualNodeNum); i++) {
            ...
            for (int j = 0; j < (substrateNodesNum + virtualNodeNum); j++) {
                if (i < substrateNodesNum && j < substrateNodesNum) {
                    if (i == j || substrate_network->getLinkBetweenNodes(
                                      allNodeIds[i], allNodeIds[j]) == nullptr) {
                        fprintf(fpDat, "%.4lf ", 0.0);
                        bVec[i][j] = 0.0;
                    } else {
                        double bw = substrate_network
                                        ->getLinkBetweenNodes(allNodeIds[i], allNodeIds[j])
                                        ->getBandwidth();
                    ...
                    if (i >= substrateNodesNum && j < substrateNodesNum) {
                        if (reachability_cond(substrate_network->getNode(allNodeIds[j]),
                                              vnr->getVN()->getNode(allNodeIds[i]),
                                              vnr->getMaxDistance())) {
                        ...
                        if (reachability_cond(substrate_network->getNode(allNodeIds[i]),
                                              vnr->getVN()->getNode(allNodeIds[j]),
                                              vnr->getMaxDistance())) {
                            fprintf(fpDat, "%d ", META_EDGE_BW);
                            bVec[i][j] = META_EDGE_BW;
        ...
        if (setBeta == true) {
            for (int i = 0; i < substrateNodesNum; i++) {
                fprintf(fpDat, "%d %.4lf\n", i,
                        substrate_network->getNode(allNodeIds[i])->getCPU());
            }
            for (int i = 0; i < virtualNodeNum; i++) {
                fprintf(fpDat, "%d %.4lf\n", substrateNodesNum + i,
                        vnr->getVN()->getNode(allNodeIds[substrateNodesNum + i])->getCPU());
        ...
        for (auto it = virtualLinkIdSet->begin(); it != virtualLinkIdSet->end(); it++) {
            auto flowSource = std::distance(
                virtualNodeIdSet->begin(),
                virtualNodeIdSet->find(vnr->getVN()->getLink(*it)->getNodeFromId()));
        ...
        for (auto it = virtualLinkIdSet->begin(); it != virtualLinkIdSet->end(); it++) {
            auto flowDestination = std::distance(
                virtualNodeIdSet->begin(),
                virtualNodeIdSet->find(vnr->getVN()->getLink(*it)->getNodeToId()));
        ...
        for (int i = 0; i < vnr->getVN()->getNumNodes(); i++) {
            ...
            for (int j = 0; j < nodesWithinReach[substrateNodeNum + i].size(); j++) {
                ...
                if (xVec[substrateNodeNum + i][current_substrate_node_local_id] >= maxValue &&
                    !(substrate_network->getNode(allNodeIds[current_substrate_node_local_id])
                          ->touched)) {
        ...
        std::vector<std::vector<double>> &xVec)
    {
        ...
        for (int i = 0; i < vnr->getVN()->getNumNodes(); i++) {
            ...
            while (randTry < 16) {
                ...
                for (int j = 0; j < nodesWithinReach[substrateNodeNum + i].size(); j++) {
                    int current_substrate_node_local_id = nodesWithinReach[substrateNodeNum + i]
                                                              .at(j);
            ...
            if (randIndex == -1) {
                while (randTry < 16) {
                    int randValue = (int)gsl_rng_uniform_int(
                        RNG::Instance()->getGeneralRNG(),
                        nodesWithinReach[substrateNodeNum + i].size());
                    int current_substrate_node_local_id = nodesWithinReach[substrateNodeNum + i]
                                                              .at(randValue);
                    if (!(substrate_network->getNode(allNodeIds[current_substrate_node_local_id])
                              ->touched)) {
                        randIndex = current_substrate_node_local_id;
                        break;
                    }
    ...
    template <>
    inline void VYVineNodeEmbeddingAlgo<>::cleanUp(
        ...
        std::shared_ptr<SUBSTRATE_TYPE> substrate_network, std::shared_ptr<VNR_TYPE> vnr)
    {
        ...
        auto it = std::copy(substrateNodeIdSet->begin(), substrateNodeIdSet->end(),
                            allNodeIds.begin());
        ...
        std::vector<std::vector<double>> flowVec(
            substrateNodesNum + virtualNodeNum,
            std::vector<double>(substrateNodesNum + virtualNodeNum, 0));
        std::vector<std::vector<double>> xVec(
            substrateNodesNum + virtualNodeNum,
            std::vector<double>(substrateNodesNum + virtualNodeNum, 0));
        char varName[50];
        ...
        for (int i = 0; i < virtualNodeNum; i++) {
            ...
            for (int j = 0; j < nodesWithinReach[substrateNodesNum + i].size(); j++) {
                {
                    tot += xVec[substrateNodesNum + i]
                               [nodesWithinReach[substrateNodesNum + i].at(j)];
                }
            }
            if (tot < Ep)
                tot = 1.0;
