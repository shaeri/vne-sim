...
namespace vineyard
{
    ...
    template <>
    inline void VYVNRProcObserver<>::setStatistics(VYStatistics &stat,
                                                   std::shared_ptr<VYVirtualNetRequest<>> req)
    {
        ...
        for (auto it = nodeIdSet->begin(); it != nodeIdSet->end(); it++) {
            ...
            if (n->getMaxCPU() == 0.0)
                continue;
            stress = (n->getMaxCPU() - n->getCPU()) / n->getMaxCPU();
            stat.std_dev_node_stress += ((stat.avg_node_stress - stress) *
                                         (stat.avg_node_stress - stress));
        }
        ...
        for (auto it = linkIdSet->begin(); it != linkIdSet->end(); it++) {
            std::shared_ptr<const VYSubstrateLink<>> l = substrate_network->getLink(*it);
            stress = (l->getMaxBandwidth() - l->getBandwidth()) / l->getMaxBandwidth();
            stat.std_dev_link_stress += ((stat.avg_link_stress - stress) *
                                         (stat.avg_link_stress - stress));
        }
        ...
        for (; i != xb.end(); i++) {
            ...
            if ((*i).port == entered_embedding_queue) {
                ...
                BOOST_LOG_TRIVIAL(info)
                    << "VY-PROC-OBSERVER: received an event on port: " << entered_embedding_queue
                    << ". A VNR has entered embedding queue.";
            } else if ((*i).port == embedding_successful) {
                stat.event_type = get_Evenet_Type_Str(
                    vne::Event_Types::EVENT_TYPE_SUCCESSFUL_EMBEDDING);
                setStatistics(stat, (*i).value);

                BOOST_LOG_TRIVIAL(info)
                    << "VY-PROC-OBSERVER: received an event on port: " << embedding_successful
                    << ". A VNR has been successfully embedded.";
            } else if ((*i).port == embedding_unsuccessful) {
                stat.event_type = get_Evenet_Type_Str(vne::Event_Types::EVENT_TYPE_FAIL_EMBEDDING);
                setStatistics(stat, (*i).value);
                BOOST_LOG_TRIVIAL(info)
                    << "VY-PROC-OBSERVER: received an event on port: " << embedding_unsuccessful
                    << ". A VNR embedding was unsuccessful.";
                ...
                BOOST_LOG_TRIVIAL(info)
                    << "VY-PROC-OBSERVER: received an event on port: " << released_resources
                    << ". A VNR left the substrate network.";
            } else {
                BOOST_LOG_TRIVIAL(info)
                    << "VY-PROC-OBSERVER: received an event on an unknown port: " << (*i).port
                    << ". A VNR left the substrate network.";
            }
