...
namespace vineyard
{
    ...
    VYVirtualNetRequest<>::VYVirtualNetRequest(
        std::shared_ptr<Network<VYVirtualNode<>, VYVirtualLink<>>> _vn, double _time,
        double _duration, int _split, int _topology, int _maxD,
          ...
          maxDistance(_maxD)
    {
        if (calcRevenue == nullptr) {
            revenue = [](const VYVirtualNetRequest<> *vnr)
                -> std::shared_ptr<std::pair<double, double>> {
                std::shared_ptr<std::pair<double, double>> rev(
                    new std::pair<double, double>(0, 0));
                if (vnr->getLinkMap()->size() != vnr->getVN()->getNumLinks() ||
                    vnr->getNodeMap()->size() != vnr->getVN()->getNumNodes()) {
                    return rev;
                }
            ...
            revenue = calcRevenue;
        if (calcCost == nullptr) {
            cost = [](const VYVirtualNetRequest<> *vnr)
                -> std::shared_ptr<std::pair<double, double>> {
                std::shared_ptr<std::pair<double, double>> cost(
                    new std::pair<double, double>(0, 0));
                if (vnr->getLinkMap()->size() != vnr->getVN()->getNumLinks() ||
                    vnr->getNodeMap()->size() != vnr->getVN()->getNumNodes()) {
                    return cost;
                ...
                for (auto it1 = vnr->getLinkMap()->begin(); it1 != vnr->getLinkMap()->end();
                     it1++) {
    ...
    {
        float mult = ConfigManager::Instance()->getConfig<float>("vineyard", "Constants",
                                                                 "revenueMultiplier");
    ...
    {
        if (ofstrm.is_open()) {
            ofstrm << vn->getNumNodes() << " " << vn->getNumLinks() << " " << split << " " << " "
                   << arrivalTime << " " << duration << " " << topology << " " << maxDistance
                   << std::endl;
            vn->writeNetworkToFile(ofstrm, false);
        } else
            BOOST_LOG_TRIVIAL(error)
                << "VYVirtualNetRequest<>::writeVNRToFile: VYThe file is not open for writing. "
                << std::endl;
    }
}  // namespace vineyard
}  // namespace vne