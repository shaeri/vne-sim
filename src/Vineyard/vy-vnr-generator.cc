...
namespace vineyard
{
    ...
    inline void VYVNRGenerator<>::setVNR()
    {
        ...
        try {
            ...
            for (int i = 0; i < numEdges; i++) {
                ...
                std::shared_ptr<VYVirtualLink<>> l(
                    new VYVirtualLink<>(bw, delay, from + nodeID_offset, to + nodeID_offset));
                vn->addLink(l);
            }
            ...
            vnr = std::shared_ptr<VYVirtualNetRequest<>>(
                new VYVirtualNetRequest<>(vn, time, duration, split, topo, maxd, revenue, cost));
          ...
          last_vnr_arrival(0.0)
    {
        ...
        vnDirPath << ConfigManager::Instance()->getConfig<std::string>("vineyard",
                                                                       "VirtualNetRequest", "path")
                  << ("/")
                  << (ConfigManager::Instance()->getConfig<std::string>(
                         "vineyard", "VirtualNetRequest", "dir"));
        const std::string fileExtension = ConfigManager::Instance()->getConfig<std::string>(
            ...
            if (exists(p) && is_directory(p)) {
                for (directory_iterator dir_iter(p); dir_iter != end_itr; ++dir_iter) {
                    if (is_regular_file(dir_iter->status()) &&
                        dir_iter->path().extension().compare(fileExtension) == 0) {
                        boost::regex rgx("/*([0-9]+)\\.txt");
                        boost::smatch match;
                        if (boost::regex_search(dir_iter->path().string().begin(),
                                                dir_iter->path().string().end(), match, rgx)) {
                            _reqFiles.push(
                                std::make_pair(atoi(match[1].str().c_str()), dir_iter->path()));
                        } else {
                            std::cerr
                                << "File could not be processed (bad name string): "
                                << dir_iter->path().string().substr(
                                       vnDirPath.str().size(), dir_iter->path().string().size());
                        }
                    }
                }
                if (_reqFiles.size() == 0) {
                ...
                std::cerr << ">>>>Exception in reading virtual network request directory.<<< \n"
                          << "Directory: "
                          << ConfigManager::Instance()->getConfig<std::string>(
                                 "vineyard", "VirtualNetwork", "dir")
                          << " does not exist in: "
                          << ConfigManager::Instance()->getConfig<std::string>(
                                 "vineyard", "VirtualNetwork", "path");
                throw;
            }
        } catch (...) {
        }
    ...
    double VYVNRGenerator<>::ta()
    {
        ...
        BOOST_LOG_TRIVIAL(info) << "---> Next arrival is in : "
                                << vnr->getArrivalTime() - last_vnr_arrival;
