...
namespace vineyard
{
    ...
    VYVineLinkEmbeddingAlgoNoDiskIO<>::VYVineLinkEmbeddingAlgoNoDiskIO()
        : LinkEmbeddingAlgorithm<Network<VYSubstrateNode<>, VYSubstrateLink<>>,
                                 VYVirtualNetRequest<>>(
              Link_Embedding_Algo_Types::WITH_PATH_SPLITTING),
          lp_problem(nullptr),
          ...
          substrateNodeIdSet(nullptr)
    {
        ...
        MCFdataFile = ConfigManager::Instance()->getConfig<std::string>("vineyard", "glpk",
                                                                        "MCFdataFile");
        MCFmodelFile = ConfigManager::Instance()->getConfig<std::string>("vineyard", "glpk",
                                                                         "MCFmodelFile");
        ...
        const std::map<int, int> *nodeIdMap)
    {
        ...
        // coeeficent vector (b)
        std::vector<std::vector<double>> bVec((substrateNodesNum),
                                              std::vector<double>(substrateNodesNum));
        ...
        for (auto it = virtualLinkIdSet->begin(); it != virtualLinkIdSet->end(); it++) {
            ...
            auto source = std::distance(substrateNodeIdSet->begin(),
                                        substrateNodeIdSet->find(sNodeFromId));
        ...
        for (auto it = virtualLinkIdSet->begin(); it != virtualLinkIdSet->end(); it++) {
            ...
            auto dest = std::distance(substrateNodeIdSet->begin(),
                                      substrateNodeIdSet->find(sNodeToId));
        ...
        for (int i = 0; i < substrateNodesNum; i++) {
            for (int j = 0; j < substrateNodesNum; j++) {
                if (i == j || substrate_network->getLinkBetweenNodes(allNodeIds[i],
                                                                     allNodeIds[j]) == nullptr) {
                    bVec[i][j] = 0.0;
                } else {
                ...
                for (int k = 0; k < numFlows; k++) {
                    for (int l = 0; l < substrateNodesNum; l++) {
                        for (int m = 0; m < substrateNodesNum; m++) {
                            int from, to, flowNum;
                            sscanf(glp_get_col_name(lp_problem, colNum), "f[f%d,%d,%d]", &flowNum,
                                   &from, &to);
                            if (flowNum == l &&
        ...
        std::map<int, std::list<std::pair<int, std::shared_ptr<Resources<double>>>>> *linkMap)
    {
        ...
        for (int i = 1; i <= numVariables; i++) {
            ...
            if (varVal > 1E-6) {
                ...
                int substrateLinkId = substrate_network
                                          ->getLinkBetweenNodes(allNodeIds[from], allNodeIds[to])
                                          ->getId();
