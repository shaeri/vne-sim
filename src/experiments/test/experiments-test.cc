...
{
    std::string vnr_dirs[] = {"reqs-12-1000-nodesMin-3-nodesMax-10-grid-25", "reqs-14-1000-nodesMin-3-nodesMax-10-grid-25"
...
for (int j = 0; j < 8; j++)
	{
		bool ret = ConfigManager::Instance()->setConfig(vnr_dirs[j], "vineyard", "VirtualNetRequest", "dir");
		assert(ret);
 		std::string str1 = "BFS-SP";
 		ret = ConfigManager::Instance()->setConfig(str1, "MCVNE", "NodeEmbeddingAlgo", "LinkEmbedder");
 ...
 BOOST_AUTO_TEST_CASE(MCVNE_NUMSIMS_TEST)
 {
     ...
     for (int i = 0; i<8; i++)
     {
     	bool ret = ConfigManager::Instance()->setConfig(numSims[i], "MCTS", "MCTSParameters", "NumSimulations");
     	assert (ret);
     	ret = ConfigManager::Instance()->setConfig(vnr_dirs[6], "vineyard", "VirtualNetRequest", "dir");
     	assert (ret);
 		std::string str1 = "BFS-SP";
 		ret = ConfigManager::Instance()->setConfig(str1, "MCVNE", "NodeEmbeddingAlgo", "LinkEmbedder");
 		assert (ret);
