...
namespace mcts
{
    MCTS::Parameters::Parameters()
        : MaxDepth(
          ...
          NumSimulations(ConfigManager::Instance()->getConfig<int>("MCTS", "MCTSParameters",
                                                                   "NumSimulations")),
          ExpandCount(
              ConfigManager::Instance()->getConfig<int>("MCTS", "MCTSParameters", "ExpandCount")),
          AutoExploration(ConfigManager::Instance()->getConfig<bool>("MCTS", "MCTSParameters",
                                                                     "AutoExploration")),
          ExplorationConstant(ConfigManager::Instance()->getConfig<double>(
              "MCTS", "MCTSParameters", "ExplorationConstant")),
          UseRave(ConfigManager::Instance()->getConfig<bool>("MCTS", "MCTSParameters", "UseRave")),
          RaveDiscount(ConfigManager::Instance()->getConfig<double>("MCTS", "MCTSParameters",
                                                                    "RaveDiscount")),
          RaveConstant(ConfigManager::Instance()->getConfig<double>("MCTS", "MCTSParameters",
                                                                    "RaveConstant")),
          DisableTree(
              ConfigManager::Instance()->getConfig<bool>("MCTS", "MCTSParameters", "DisableTree")),
          UseSinglePlayerMCTS(ConfigManager::Instance()->getConfig<bool>("MCTS", "MCTSParameters",
                                                                         "UseSinglePlayerMCTS")),
          SPMCTSConstant(ConfigManager::Instance()->getConfig<double>("MCTS", "MCTSParameters",
                                                                      "SPMCTSConstant")),
          ParalleizationType(ConfigManager::Instance()->getConfig<int>("MCTS", "MCTSParameters",
                                                                       "ParallelizationType"))
    {
    }
    MCTS::MCTS(const std::shared_ptr<MCTSSimulator> sim)
        : simulator(sim), params(Parameters()), treeDepth(0), history(vector<int>())
    {
    ...
    bool MCTS::update(int action, double reward)
    {
        ...
        BOOST_LOG_TRIVIAL(debug) << "Child's node value: " << child_node->value.getValue()
                                 << std::endl;
    ...
    int MCTS::selectAction()
    {
        if (params.DisableTree)
            ...
            if (params.ParalleizationType == 0) {
                ...
                MPI::COMM_WORLD.Allreduce(childrenCounts, childrenCountsGlobal, numChildren,
                                          MPI::INT, customSumOp);
                MPI::COMM_WORLD.Allreduce(childrenValues, childrenValuesGlobal, numChildren,
                                          MPI::DOUBLE, customSumOp);
 ...
 */
                for (int i = 0; i < numChildren; i++) {
                    root->child(i)->value.set(
                        childrenCountsGlobal[i],
                        childrenValuesGlobal[i] / (double)childrenCountsGlobal[i]);
                }
            }
#endif
        }
    ...
    double MCTS::simulateNode(std::shared_ptr<TreeNode> node)
    {
        ...
        if (child_node->getState() == nullptr && !terminal &&
            node->value.getCount() >= params.ExpandCount)
            *child_node = *(expandNode(st));
...
*/
        for (int action = mystart; action < myend; action++) {
            ...
            if (params.UseSinglePlayerMCTS && child_node->value.getCount() > 0 &&
                ...
                q += sqrt((child_node->value.getSumSquaredValue() -
                           n * child_node->value.getValue() * child_node->value.getValue() +
                           params.SPMCTSConstant) /
                          n);
