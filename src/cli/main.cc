...
int run_experiment(string vnr_file, string algo)
{
    if (algo == "mcvne_bfs_mcf") {
        bool ret = ConfigManager::Instance()->setConfig<std::string>(vnr_file, "vineyard",
                                                                     "VirtualNetRequest", "dir");
        assert(ret);
        string str1 = "BFS-SP";
        ret = ConfigManager::Instance()->setConfig<std::string>(str1, "MCVNE", "NodeEmbeddingAlgo",
                                                                "LinkEmbedder");
        assert(ret);
        ...
        bool ret = ConfigManager::Instance()->setConfig(vnr_file, "vineyard", "VirtualNetRequest",
                                                        "dir");
        assert(ret);
        ...
        ret = ConfigManager::Instance()->setConfig<std::string>(str1, "vineyard", "Configs",
                                                                "nodeMappingType");
        assert(ret);
        vne::experiments::VineNodeMCFLinkExp<> exp = vne::experiments::VineNodeMCFLinkExp<>();
...
int main(int argc, char *argv[])
{
    ...
    desc.add_options()("help,h", "help message here")("config", po::value<string>(),
    ...
    subcmds.add("subcmd", 1).add("subargs", -1);
    ...
    if (opts["subcmd"].as<string>() == "simulate") {
        ...
        simulate_opts.add_options()("algorithm,a", po::value<string>(),
                                    "An algorithm: mcvne_bfs_mcf, mcvne_mcf_mcf, mcvne_bfs_bfs, "
                                    "grc_mcf, grc_bfs, vineyard_d, vineyard_r")(
            "vnr_dir,r", po::value<string>(),
            "The directory name (within the VNR directory) that holds the VNRs");

        vector<string> remaining = po::collect_unrecognized(parsed.options,
        ...
        int ret = run_experiment(simulate_vars["vnr_dir"].as<string>(),
                                 simulate_vars["algorithm"].as<string>());
