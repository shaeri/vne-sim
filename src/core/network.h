...
class Network<NodeC<NodeT...>, LinkC<LinkT...>>
{
   ...
   public:
    ...
    const std::shared_ptr<const std::vector<std::shared_ptr<LinkC<LinkT...>>>> getLinksForNodeId(
        int id) const;
    ...
    const std::shared_ptr<std::vector<std::shared_ptr<NodeC<NodeT...>>>> getNodesWithConditions(
        T &...args);
    //template<typename CONDITION, typename ... T>
...
template <typename... NodeT, template <typename...> class NodeC, typename... LinkT,
          template <typename...> class LinkC>
void Network<NodeC<NodeT...>, LinkC<LinkT...>>::addNode(std::shared_ptr<NodeC<NodeT...>> node)
{
    auto it = nodesMap.find(node->getId());
    assert(it == nodesMap.end() && "A node with the same ID exists in the network.");
...
template <typename... NodeT, template <typename...> class NodeC, typename... LinkT,
          template <typename...> class LinkC>
void Network<NodeC<NodeT...>, LinkC<LinkT...>>::addLink(std::shared_ptr<LinkC<LinkT...>> link)
{
    assert(nodesMap.find(link->getNodeFromId()) != nodesMap.end() &&
           "Trying to add a link with invalid nodefrom ID");
    assert(nodesMap.find(link->getNodeToId()) != nodesMap.end() &&
           "Trying to add a link with invalid nodeTo ID");
    assert(linksMap.find(link->getId()) == linksMap.end() &&
           "link id already exists in the network.");
    //add an element for node From
    BOOST_LOG_TRIVIAL(debug) << "Link ID: " << link->getId()
                             << " from node: " << link->getNodeFromId()
                             << " to: " << link->getNodeToId();
    ...
    if (it == connectionMap.end()) {
        ...
        connectionMap.insert(
            std::make_pair(link->getNodeFromId(), std::make_shared<std::vector<int>>(v)));
    ...
    if (it == connectionMap.end()) {
        ...
        v.push_back(link->getId());
        connectionMap.insert(
            std::make_pair(link->getNodeToId(), std::make_shared<std::vector<int>>(v)));
    } else
...
template <typename... NodeT, template <typename...> class NodeC, typename... LinkT,
          template <typename...> class LinkC>
inline const std::shared_ptr<NodeC<NodeT...>> Network<NodeC<NodeT...>, LinkC<LinkT...>>::getNode(
    int id) const
{
...
{
    const std::shared_ptr<std::vector<std::shared_ptr<LinkC<LinkT...>>>> v(
        new std::vector<std::shared_ptr<LinkC<LinkT...>>>());
...
Network<NodeC<NodeT...>, LinkC<LinkT...>>::getLinkBetweenNodes(int nodeIdFrom, int nodeIdTo) const
{
    ...
    if (connectionMap.size() > 0 &&
        (nodeFromItr == connectionMap.end() || nodeToItr == connectionMap.end()))
...
template <typename... NodeT, template <typename...> class NodeC, typename... LinkT,
          template <typename...> class LinkC>
const std::shared_ptr<LinkC<LinkT...>> Network<NodeC<NodeT...>, LinkC<LinkT...>>::getLink(
    int id) const
{
...
{
    const std::shared_ptr<std::vector<std::shared_ptr<NodeC<NodeT...>>>> vec(
        new std::vector<std::shared_ptr<NodeC<NodeT...>>>());
    for (auto it = nodesMap.begin(); it != nodesMap.end(); ++it)
...
{
    const std::shared_ptr<std::vector<std::shared_ptr<LinkC<LinkT...>>>> vec(
        new std::vector<std::shared_ptr<LinkC<LinkT...>>>());
    for (auto it = linksMap.begin(); it != linksMap.end(); ++it)
        vec->push_back(it->second);
    return vec;
...
template <typename... NodeT, template <typename...> class NodeC, typename... LinkT,
          template <typename...> class LinkC>
inline void Network<NodeC<NodeT...>, LinkC<LinkT...>>::writeNetworkToFile(std::ofstream &ofstrm,
                                                                          bool writeNetworkSize)
{
    if (ofstrm.is_open()) {
        if (writeNetworkSize)
        ...
        BOOST_LOG_TRIVIAL(error) << "Network::writeVNRToFile: VYThe file is not open for writing. "
                                 << std::endl;
}
...
{
    const std::shared_ptr<std::vector<std::shared_ptr<const NodeC<NodeT...>>>> vec(
        new std::vector<std::shared_ptr<const NodeC<NodeT...>>>());
    CONDITION cond;
    for (auto it = nodesMap.begin(); it != nodesMap.end(); ++it) {
