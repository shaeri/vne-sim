...
std::shared_ptr<RNG> RNG::Instance()
{
    if (_instance == nullptr) {
        _instance = std::shared_ptr<RNG>(
            new RNG(ConfigManager::Instance()->getConfig<unsigned long int>("core", "rngSeed")));
    }
    return _instance;
}
...
{
    useSameSeedForParallelRuns = ConfigManager::Instance()->getConfig<bool>(
        "core", "rngUseSameSeedForParallelRuns");
    if (useSameSeedForParallelRuns)
        seed = _seed;
    else
        seed = _seed + MPI::COMM_WORLD.Get_rank();
#else
RNG::RNG(unsigned long int _seed) : seed(_seed)
{
#endif
