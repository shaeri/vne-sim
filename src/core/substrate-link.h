...
public:
    ...
    Embedding_Result embedLink(std::shared_ptr<VirtualLink<LINKRES...> > _l, std::shared_ptr<Resources<LINKRES...>> _res);
...
template<typename... LINKRES>
Embedding_Result SubstrateLink<LINKRES...>::embedLink (std::shared_ptr<VirtualLink<LINKRES...> > _l, std::shared_ptr<Resources<LINKRES...>> _res)
{
    BOOST_LOG_NAMED_SCOPE("SubstrateLink::embedLink");
    auto it = embedded_links.find(_l->getId());
    assert(it == embedded_links.end());
