    ...
    namespace grc {
        ...
        GRCNodeEmbeddingAlgo<>::calculateSubstrateGRCVector (std::shared_ptr<SUBSTRATE_TYPE> substrate_network)
        {
            ...
            for(auto it1 = substrateIdSet->begin(); it1 != substrateIdSet->end(); it1++)
            {
                ...
                {
                    std::shared_ptr<VYSubstrateLink<>> linkBetweenSNs = substrate_network->getLinkBetweenNodes(*it1, *it2);
                    ...
                    {
                        std::shared_ptr<const std::vector<std::shared_ptr<VYSubstrateLink<>> >>linksConnectedToSN = substrate_network->getLinksForNodeId(*it2);
                        double sumBW = 0.0;
        ...
        GRCNodeEmbeddingAlgo<>::calculateVNRGRCVector (std::shared_ptr<VNR_TYPE> vnr)
        {
            ...
            for(auto it1 = vnIdSet->begin(); it1 != vnIdSet->end(); it1++)
            {
                for (auto it2 = vnIdSet->begin(); it2 != vnIdSet->end(); it2++)
                {
                    ...
                    {
                        std::shared_ptr<const std::vector<std::shared_ptr<VYVirtualLink<>> >>linksConnectedToVN = vnr->getVN()->getLinksForNodeId(*it2);
                        double sumBW = 0.0;
